...
def irods_mkdir_p(irods_session: iRODSSession, collection_path: str):
    try:
        if dry_run:
            print(f"dry-run: would create collection {collection_path} !!!! CHECK THIS")
        else:
...
def do_initial_sync_and_or_restart(
    ...
    for path_object in path_objects:
        if path_object.is_file() and (full_path := path_object.absolute()):
            ...
            if path_object.is_dir() and (full_path := path_object.absolute()):
                print(f" try out to handle dirs {full_path}", verbosity=2)
                try:
                    rel_local_path = full_path.relative_to(path)
                    irods_path = str(pathlib.PurePath(destination, str(rel_local_path)))
                    irods_collection = irods_session.collections.get(irods_path)
                    print ( f"iRODS Collection {irods_collection.name} is available", verbosity=2)
                except irods.exception.CollectionDoesNotExist:
                    print ( f"iRODS Collection '{rel_local_path}' NOT available", verbosity=2)
                    irods_mkdir_p(
                        irods_session,
                        irods_path
                    )
        ...
        if path_object.is_file() and (full_path := path_object.absolute()):
            ...
            if check_filters(
                ...
                try:
                    ...
                    if update_metadata:
                        ...
                        '''
                        NIET NODIG OM METADATA VOOR FILES TE UPDATEN ????
                        if metadata_handlers:
                            ctx["sync"] = True
                            metadata_dict = {}
                            for metadata_handler, kwargs in metadata_handlers:
                                    if "verbosity" in kwargs:
                                        kwargs["verbosity"] = verbosity_level
                                    if "irods_session" in kwargs:
                                        kwargs["irods_session"] = irods_session
                                    if "kwargs_from_ctx" in kwargs:
                                        for name ,values in vars().items():
                                            for arg in kwargs["kwargs_from_ctx"]:
                                                if name == arg:
                                                    kwargs[arg] = values
                                                else:
                                                    if arg in ctx:
                                                        kwargs[arg] = ctx[arg] 
                                    metadata_dict |= metadata_handler(str(full_path), **kwargs)
                            if metadata_dict:
                                bulk_add_metadata(  item=irods_data_object, metadata_items=metadata_dict, prefix="mg." )
                                print(
                                    f"Added {len(metadata_dict)} metadata items to {irods_data_object.name}"
                                )
                        '''
                        continue
                    else:
            ...
            if path_object.is_dir() and (full_path := path_object.absolute()):
                ...
                try:
                    ...
                    print ( f"iRODS Collection {irods_collection.name} is available", verbosity=2)
                    if update_metadata:
                        print( f"UPDATE metadata for {irods_collection.name} ", verbosity=4)
                        if metadata_handlers:
                                ctx["sync"] = True
                                metadata_dict = {}
                                for metadata_handler, kwargs in metadata_handlers:
                                    ...
                                    metadata_dict |= metadata_handler(str(full_path), **kwargs)
                                if metadata_dict:
                                    bulk_add_metadata(  item=irods_collection, metadata_items=metadata_dict, prefix="mg." )
                                    print( 
                                        f"Added {len(metadata_dict)} metadata items to {irods_collection.name}"
                                    )                           
                except irods.exception.CollectionDoesNotExist:
                    print ( f"iRODS Collection '{rel_local_path}' NOT available", verbosity=1)
...
@click.option("-um","--update_metadata", is_flag=True, help="Update all metadata without uploading files or folders")
...
def mango_ingest(
    ...
    if ctx.invoked_subcommand is None:
        ...
        sync_glob = None
        if sync or no_watch or update_metadata:
